...
func main() {
	...
	q, err := qp.NewParse(url.Query(), qp.Validations{
		...
		"limit:required": qp.MinMax(10, 100),  // limit must present in the Query part and must be between 10 and 100 (default: Min(1))
		"sort":           qp.In("id", "name"), // sort could be or not in the query but if it is present it must be equal to "in" or "name"

		"s":      qp.In("one", "two"), // filter: s - string and equal
	...
	q.AddValidation("fields", qp.In("id", "name"))
